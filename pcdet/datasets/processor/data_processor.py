...
from .geospa_features import compute_geospa_features_batch
...
class DataProcessor(object):
    ...
    def compute_geospa_features(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.compute_geospa_features, config=config)

        points = data_dict['points']
        k = config.get('K_NEIGHBORS', 16)

        geospa = compute_geospa_features_batch(points, k=k)
        # Append [scatterness, linearness, surfaceness] to point features
        data_dict['points'] = np.concatenate([points, geospa], axis=1)
        return data_dict
